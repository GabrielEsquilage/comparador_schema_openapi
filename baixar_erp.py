...
def baixar_json(url: str, prefixo: str = "api-docs-erp-data", tentativas: int = 3, timeout: int = 120):
    diretorio_erp = os.path.join(os.getcwd(), "erp")
    if not os.path.exists(diretorio_erp):
        os.makedirs(diretorio_erp)
    ...
    for tentativa in range(1, tentativas + 1):
        try:
            ...
            caminho_arquivo = os.path.join(diretorio_erp, nome_arquivo)

            with open(caminho_arquivo, "w", encoding="utf-8") as f:
                f.write(resposta.text)

            print(f"✅ JSON salvo em: {caminho_arquivo}")
            limpar_arquivos_antigos(diretorio_erp)
...
if __name__ == "__main__":
    url = "https://erp-api-stage-52421872894.us-central1.run.app/v3/api-docs"
    baixar_json(url)
